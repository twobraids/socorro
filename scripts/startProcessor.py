...
# the following section is a translation of old-style configuration into the
# new style configuration.  It lets a new style app run without leaving the
...
# converter: str
trans_config.destination.storage0.database_password = config.databasePassword
...
trans_config.destination.storage0.transaction_executor_class = \
    socorro.database.transaction_executor.TransactionExecutorWithLimitedBackoff
...
# converter: str
trans_config.logging.stderr_line_format_string = config.stderrLineFormatString
...
# converter: str
trans_config.logging.syslog_line_format_string = config.syslogLineFormatString
...
# doc: the number of frames to keep in the raw dump at the tail of the frame
#      list
# converter: int
...
# doc: the name of the user within the database
# converter: str
trans_config.processor.database_user = config.databaseName

# name: processor.known_flash_identifiers
# doc: A subset of the known "debug identifiers" for flash versions, associated
#      to the version
# converter: json.loads
trans_config.processor.known_flash_identifiers = config.knownFlashIdentifiers

# name: processor.minidump_stackwalk_pathname
# doc: the full pathname of the extern program minidump_stackwalk (quote path
...
# doc: a regular expression matching frame signatures that should be ignored
#      when generating an overall signature
# converter: str
...
#      coupled with the following frame signature when generating an overall
#      signature
# converter: str
...
# doc: a list of frame signatures that should always be considered top of the
#      stack if present in the stack
# converter: eval
...
# converter: int
trans_config.producer_consumer.idle_delay = config.processorLoopTime

# name: producer_consumer.maximum_queue_size
# doc: the maximum size of the internal queue
# converter: int
trans_config.producer_consumer.maximum_queue_size = config.numberOfThreads * 2
...
trans_config.registrar.check_in_frequency = config.processorCheckInFrequency
...
# doc: the id number for the processor (must already exist) (0 for create new
#      Id, "auto" for autodetection, "host" for same host)
# converter: str
