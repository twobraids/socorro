...
class MonitorApp(App):
    ...
    def _job_cleanup_thread(self):
        ...
        does two transactions: the first deletes completed jobs; the second
        kicks stalled jobs."""
    ...
    def _sweep_dead_processors_transaction(self, connection):
            ...
            if dead_processors:
                ...
                if not live_processors:
                    if self.config.registrar.quit_if_no_processors:
                        raise NoProcessorsRegisteredException(
        ...
        except NoProcessorsRegisteredException:
            self.quit = True
            self.config.logger.critical('there are no live processors')

    #--------------------------------------------------------------------------
    @staticmethod
    def _compare_second_of_sequence(x, y):
        return cmp(x[1], y[1])
    ...
    def _balanced_processor_iter(self):
        ...
        try:
            ...
            while True:
                ...
                list_of_processors_and_loads.sort(
                  MonitorApp._compare_second_of_sequence
                )
