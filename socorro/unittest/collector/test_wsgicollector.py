...
class TestProcessorApp(unittest.TestCase):
    ...
    def test_POST(self):
        ...
        with mock.patch('socorro.collector.wsgicollector.web') as mocked_web:
            mocked_web.input.return_value = form
            with mock.patch('socorro.collector.wsgicollector.web.webapi') \
                    as mocked_webapi:
                mocked_webapi.rawinput.return_value = rawform
                with mock.patch('socorro.collector.wsgicollector.utc_now') \
                        as mocked_utc_now:
                    mocked_utc_now.return_value = datetime(
                        2012, 5, 4, 15, 10
                        )
                    with mock.patch('socorro.collector.wsgicollector.time') \
                            as mocked_time:
            ...
            with mock.patch('socorro.collector.wsgicollector.web.webapi') \
                ...
                with mock.patch('socorro.collector.wsgicollector.utc_now') \
                    ...
                    with mock.patch('socorro.collector.wsgicollector.time') \
                        ...
                        self.assertFalse(
                            self.crash_storage.save_raw.call_count
                        )
