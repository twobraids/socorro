...
#testBaseDate = dt.datetime(2008,1,1,1,1,1,1,tzinfo=utctz)
#testFirstIntervalBegin = dt.datetime(2008,1,1,tzinfo=utctz)
...
#dataForStartDateTest = [
  #{'count':18,'uptime':64.5,'last_updated':dt.datetime(2009,3,3,3,3,3,tzinfo=utctz), 'productdims_id':1,'osdims_id':1,'signature':'_PR_MD_SEND','hang_count':0,'plugin_count':0},
  #{'count':10,'uptime':56.4,'last_updated':dt.datetime(2009,3,3,3,3,5,tzinfo=utctz), 'productdims_id':2,'osdims_id':1,'signature':'nsAutoCompleteController::ClosePopup','hang_count':10,'plugin_count':10},
  #{'count':1, 'uptime':45.6,'last_updated':dt.datetime(2009,3,3,3,3,7,tzinfo=utctz), 'productdims_id':3,'osdims_id':2,'signature':'nsFormFillController::SetPopupOpen','hang_count':0,'plugin_count':0},
  #]
...
#def genBuild():
  #builds = [('200712312355',dt.datetime(2007,12,31,23,55,tzinfo=utctz)),('200712302355',dt.datetime(2007,12,30,23,55,tzinfo=utctz)),('200712292355',dt.datetime(2007,12,29,23,55,tzinfo=utctz)),]
  #while True:
    #for b in builds:
      #yield b
...
#class TestTopCrashesBySignature(unittest.TestCase):
  ...
  #def prepareExtractDataForPeriod(self, columnName, sizePerDay=2, numDays=5):
    ...
    #minStamp = dt.datetime(2100,11,11,tzinfo=utctz)
    #maxStamp = dt.datetime(1900,11,11,tzinfo=utctz)
    #for d in data:
      #ds = d[columnName]
  ...
  #def testConstructor(self):
    ...
    #config['startDate'] = dt.datetime(2009,01,01,0,0,1,tzinfo=utctz)
    #assert_raises(SystemExit,topcrasher.TopCrashesBySignature,config)

    #config = copy.copy(me.config)
    #config['endDate'] = dt.datetime(2009,01,01,0,0,1,tzinfo=utctz)
    #assert_raises(SystemExit,topcrasher.TopCrashesBySignature,config)
  ...
  #def testStoreFacts(self):
    ...
    #stime = dt.datetime(2009,9,8,7,30,tzinfo=utctz)
    ...
    #sum = dict((k,summaryCrashes[k]) for k in smKeys)
    #cd = tc.fixupCrashData(sum,dt.datetime(2009,9,8,7,30,tzinfo=utctz),fifteen)
    ...
    #cd = tc.fixupCrashData(summaryCrashes,dt.datetime(2009,9,8,7,30,tzinfo=utctz),fifteen)
  ...
  #def testChangeWindowSize(self):
    ...
    #minStamp, maxStamp, data = self.prepareExtractDataForPeriod('date_processed',31,5) # full set of keys
    #bogusEnd = dt.datetime(2007,12,30,tzinfo=utctz)
    ...
    #config = copy.copy(me.config)
    #config['startDate'] = dt.datetime(2008,1,2,1,tzinfo=utctz)
    #config['endDate'] = dt.datetime(2008,1,2,2,tzinfo=utctz)
    ...
    #config['endDate'] = dt.datetime(2008,1,2,3,tzinfo=utctz)
    #config['startWindow'] = config['startDate']
  ...
  #def testProcessDateInterval(self):
    ...
    #tc = topcrasher.TopCrashesBySignature(me.config)
    #config = copy.copy(me.config)
    #config['startWindow'] = dt.datetime(2008,1,2,0,0,tzinfo=utctz)
    #config['deltaWindow'] = dt.timedelta(minutes=12)
    #config['startDate'] =  dt.datetime(2008,1,2,0,0,tzinfo=utctz)
    #config['endDate'] = dt.datetime(2008,1,6,5,0,tzinfo=utctz)
    #tc = topcrasher.TopCrashesBySignature(config)
