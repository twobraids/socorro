...
from socorro.processor.legacy_processor import create_symbol_path_str
from socorro.processor.hybrid_processor import HybridCrashProcessor
    ...
    def test_analyze_header(self):  # verify fix for Bug 881623 in test one
        ...
        with mock.patch(mocked_transform_rules_str) as m_transform_class:
            ...
            with mock.patch(utc_now_str) as m_utc_now:
                m_utc_now.return_value = datetime(2012, 5, 4, 15, 11,
                                                  tzinfo=UTC)
                hyb_proc = HybridCrashProcessor(config, config.mock_quit_fn)
                ...
                result = hyb_proc._analyze_header(
