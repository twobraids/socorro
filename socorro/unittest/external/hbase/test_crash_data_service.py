...
from socorro.external import MissingArgumentError, ResourceNotFound, \
                             ResourceUnavailable
from socorro.external.hbase import crashstorage, hbase_client
from socorro.external.hbase.crash_data import CrashData
from socorro.external.postgresql.util_service import Util
from socorro.external.rabbitmq.priorityjobs_service import Priorityjobs
...
class TestIntegrationHBaseCrashData(TestCase):
    ...
    def setUp(self):
        ...
        overrides = {
            'services.Util.cls': Mock(),
            'services.Priorityjobs.cls': Mock()
        }
        self.config_manager = get_standard_config_manager(
            service_classes=(CrashData, Util, Priorityjobs),
            overrides=overrides,
        )
    ...
    def test_get(self):
        with self.config_manager.context() as config:
            ...
            util_mock = Mock()
            config.services['Util.cls'] = util_mock

            service = config.services.CrashData.cls(
                config=config,
            )
            params = {
                'datatype': 'raw',
