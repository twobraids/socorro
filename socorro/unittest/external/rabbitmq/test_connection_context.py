...
class TestConnection(unittest.TestCase):
    ...
    def test_close(self):
        ...
        conn.close()
        self.assertTrue(faked_connection_object.close.called_once_with())
...
class TestConnectionContext(unittest.TestCase):
    ...
    def _setup_config(self):
        ...
        config.standard_queue_name = '5672'
        config.priority_queue_name = '5672'
    ...
    #--------------------------------------------------------------------------
    def test_constructor(self):
        conn_context = ConnectionContext(self._setup_config)
        self.assertTrue(conn_context.config is conn_context.local_config)
    ...
    def test_connection(self):
        ...
        with patch(pika_string) as mocked_pika_module:
            conn_context = ConnectionContext(config)
            conn = conn_context.connection()
            self.assertTrue(
                mocked_pika_module.credentials.PlainCredentials \
                    .called_once_with('guest', 'guest')
            )
            self.assertTrue(
                mocked_pika_module.ConnectionParameters.called_once_with(
                    host=conn_context.config.host,
                    port=conn_context.config.port,
                    virtual_host=conn_context.config.virtual_host,
                    credentials=mocked_pika_module.credentials. \
                        PlainCredentials.return_value
                )
            )
            self.assertTrue(
                mocked_pika_module.BlockingConnection.called_one_with(
