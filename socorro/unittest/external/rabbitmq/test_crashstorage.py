...
class TestCrashStorage(unittest.TestCase):
    ...
    def test_save_raw_crash(self):
        ...
        crash_store.save_raw_crash(raw_crash=DotDict(),
                                   dumps=DotDict(),
                                   crash_id='crash_id')
        config.logger.debug.assert_called_once_with(
        'RabbitMQCrashStorage legacy_processing key absent in crash %s', 
        'crash_id'
        )
        ...
        config.logger.debug.assert_called_once_with(
            'RabbitMQCrashStorage not saving crash %s, legacy processing flag is %d',
            'crash_id',
            5
        )
    ...
    def test_ack_crash(self):
        config = self._setup_config()
        crash_store = RabbitMQCrashStorage(config)
        crash_store.acknowledgement_queue = Mock()
        
        crash_store.ack_crash('crash_id')
        
        crash_store.acknowledgement_queue.put.assert_called_once_with('crash_id')