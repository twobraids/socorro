...
class BreakpadStackwalkerRule2015(ExternalProcessRule):
    ...
    required_config = Namespace()
    required_config.change_default(
        ExternalProcessRule,
        'command_line',
        default=(
             'timeout -s KILL 30 {minidump_stackwalk_pathname} '
        ...
        represses the stderr output.
              ...
              input parameters:
                dump_pathname: the complete pathname of the dumpfile to be
                                  analyzed
        """
        local_config = dict(self.config)  # make a local shallow copy
        local_config['dump_pathname'] = dump_pathname
        local_config['raw_crash_pathname'] = raw_crash_pathname
        command_line = self.config.stackwalk_command_line.format(
            **self.local_config
        )
