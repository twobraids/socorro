...
class HybridCrashProcessor(RequiredConfig):
    ...
    required_config.add_option(
        ...
        default='timeout -s KILL 30 $minidump_stackwalk_pathname --raw-json $rawfilePathname $dumpfilePathname '
    ...
    def __init__(self, config, quit_check_callback=None):
        ...
        self.rule_system.raw_crash_transform = self._load_transform_rules(
            ...
            'socorro.processor.raw_crash_rewrite.default_classifier_rules',
            'apply_all_rules',
        )
        self.rule_order.append(self.rule_system.raw_crash_transform)

        self.rule_system.processed_crash = self._load_transform_rules(
            "processor.processed_crash",
            'socorro.processor.processed_crash.default_classifier_rules',
            'apply_all_rules',
        )
        self.rule_order.append(self.rule_system.processed_crash)
        ...
        self.rule_order.append(self.rule_system.skunk_classifier)

        self.rule_system.support_classifier = self._load_transform_rules(
            "processor.support_classifiers",
            'socorro.processor.support_classifiers.default_classifier_rules',
            'apply_until_action_succeeds',
        )
        self.rule_order.append(self.rule_system.support_classifier)
    ...
    def _stackwalk_analysis(
        ...
        with closing(dump_analysis_line_iterator) as mdsw_iter:
            ...
            # -----------------------------------------------------------------
            # gather the PIPE dump for storage
            try:
            ...
            # -----------------------------------------------------------------
            # collect the json dump for analysis
            for x in mdsw_iter:
            ...
            # -----------------------------------------------------------------
            # extract values for legacy positions within the processed dump
            try:
    ...
    #--------------------------------------------------------------------------
    def _load_transform_rules(
        self,
        rule_category,
        default_rule_location,
        rule_executor_method
    ):
        #sql = (
        ...
        #except Exception:
            #self.config.logger.warning(
                #'Unable to load transform rules from the database, falling'
                #' back to defaults',
                #exc_info=True
            #)
            #rules = []
