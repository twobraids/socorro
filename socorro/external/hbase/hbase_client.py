...
class HBaseConnectionForCrashReports(HBaseConnection):
    ...
    def get_dump(self, ooid, name=None):
        ...
        If the ooid doesn't exist, raise not found"""
        if not name or name == 'upload_file_minidump':
            name = 'dump'
    ...
    def _make_dump_name(family_qualifier):
        """take the HBase column family and qualifier and split off the
        family leaving only the qualifier.  This is to become the name
        of the dump.  There are inconsistencies in naming throughout the
        system.  It the collector it's called 'upload_file_minidump' while
        in HBase its called 'dump'.  Since the name is set in breakpad,
        it is canonnical.  'dump' --> 'upload_file_minidump'"""
    ...
    def get_dumps(self, ooid):
        ...
        try:
            if not listOfRawRows:
                raise OoidNotFoundException(ooid)
            column_mapping = listOfRawRows[0].columns
            d = dict((self._make_dump_name(k), v.value)
    ...
    def put_json_dump(self, ooid, json_data, dumps,
        ...
        for key, dump in dumps.iteritems():
            if not key or key == 'upload_file_minidump':
                key = 'dump'
