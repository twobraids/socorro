...
class FetchTransformSaveApp(App):
    ...
    # fts_impl for the crashmover reads raw_crash and raw dumps from the
    # source and then just turns around and writes them to the destination.
    # On the other hand, the processor's fts_impl reads raw_crash, raw_dumps
    # and any exsiting processed crash, applies the processor transformation
    # to them (Processor2015 Rules) and then saves them to the destination.
    # There are a family of fts_impl classes, that read/write various
    # combinations of the raw_crashes, raw_dumps and processed crashes. This
    # class is key to getting the FTS apps to do just about anything, even
    # outside the domain of crash reporting.
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    def worker_method(
        ...
        app.  It calls out to the fts-impl class """
        try:
    ...
    def _setup_source_and_destination(self, transform_fn=None):
        ...
        try:
            if self.config.dry_run:
                self.source = DryRunCrashStorage(self.config.source)
            else:
