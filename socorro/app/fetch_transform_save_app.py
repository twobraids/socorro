...
class FetchTransformSaveApp(App):
    ...
    def __init__(self, config):
        ...
        if config.number_of_submissions == 'forever':
            self.source_iterator = self._infinite_iterator
        elif config.number_of_submissions == 'all':
            self.source_iterator = self._all_iterator
        else:
            self.source_iterator = self._limited_iterator
    ...
    def _basic_iterator(self):
        ...
        for x in self._create_iter():
            if x is None:
                ...
                arg_kwarg_form = ((x,), {})  # (args, kwargs)
                self.config.logger.debug('yielding %s', arg_kwarg_form)
                yield arg_kwarg_form
